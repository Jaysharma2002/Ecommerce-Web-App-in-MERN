...
function AdminOrder(){
    ...
    useEffect(()=>{
        const fetch=async()=>{
            const response=await axios.get('https://ecommerce-web-app-in-mern-1.onrender.com/api/product/getorder')
    ...
    useEffect(()=>{
        if(selectedOrders && updateButton)
        {
            const fetch=async()=>{
                ...
                const response=await axios.put('https://ecommerce-web-app-in-mern-1.onrender.com/api/product/updatestatus',{selectedOrders:selectedOrders,changeOption:{orderStatus:changeOption}})
                console.log(response.data)
                ...
                <div className="Adminorder">
                    ...
                    <div className="statuscontainer">
                        ...
                        <select onChange={(e)=>setOption(e.target.value)} className="newoption">
                            <option value="Placed" className="newoption">Placed</option>
                            <option value="Dispatched" className="newoption">Dispatched</option>
                            <option value="Failed" className="newoption">Failed</option>
                        </select>
                    </div>
                    <div className="statuscontainer">
                        ...
                        <select onChange={(e)=>setChangeOption(e.target.value)} className="newoption">
                            <option value="Placed" className="newoption">Placed</option>
                            <option value="Dispatched" className="newoption">Dispatched</option>
                            <option value="Delivered" className="newoption">Delivered</option>
                            <option value="Failed" className="newoption">Failed</option>
                        </select>
                    </div>
                </div>
